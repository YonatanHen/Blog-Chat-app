...
| Render Key Value (Redis) | 🚧 declared in `render.yaml`, not yet provisioned | P1 (sessions) → P4 (chat buffer) → P6 (rate limiting). Ephemeral by design. Presence is in-memory, not Redis — see the realtime chat design §4 |
